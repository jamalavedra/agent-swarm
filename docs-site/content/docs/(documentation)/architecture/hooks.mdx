...
- **Session summary + LLM memory rating** — Runs a lightweight model (Haiku) to extract key learnings from the session transcript. When `MEMORY_LLM_RATER_ENABLED=true`, the same Haiku call also produces structured `useful: true | false` ratings for the memories that were retrieved into the task's prompt — piggybacking on the summary call so there's no extra LLM round-trip. Ratings are POSTed to `/api/memory/rate` with `source: "llm"` and feed the memory rater pipeline.
