...
**Supported today:** `claude` (Anthropic Claude Code CLI), `pi` (pi-mono, in-process via `@mariozechner/pi-coding-agent`), `codex` (OpenAI Codex SDK), `claude-managed` (Anthropic Managed Agents — sessions execute in Anthropic's cloud sandbox).
