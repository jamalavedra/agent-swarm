...
| `SECRETS_ENCRYPTION_KEY` | — | Base64-encoded 32-byte AES-256-GCM key for encrypting `swarm_config` secrets at rest. See [Secrets Encryption](#secrets-encryption) |
| `SECRETS_ENCRYPTION_KEY_FILE` | — | Path to a file containing the base64 encryption key (alternative to inline env var). Useful with Docker secrets |
...
|----------|----------|---------|-------------|
| `HARNESS_PROVIDER` | No | `claude` | AI provider: `claude` (Claude Code), `codex`, or `pi` (pi-mono). See [Harness Configuration](/docs/guides/harness-configuration) |
