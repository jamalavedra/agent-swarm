...
### Explicit Slack Context Propagation

When programmatically delegating tasks from a Slack thread (e.g., lead breaking down work for workers), you can pass Slack metadata directly on `send-task`:
...
This allows explicit Slack context propagation without requiring `parentTaskId` (which implies session continuity). Workers receiving these tasks will automatically post progress updates back to the originating Slack thread.
