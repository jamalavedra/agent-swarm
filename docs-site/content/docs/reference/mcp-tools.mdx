...
Slack metadata (`slackChannelId`, `slackThreadTs`) is now auto-inherited from the creator's current task via `X-Source-Task-Id` header — no need to pass explicitly.
